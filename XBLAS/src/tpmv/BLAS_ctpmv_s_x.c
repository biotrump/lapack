 ...
 */
{
  const char *routine_name = "BLAS_ctpmv_s_x";
  ...
  case blas_prec_indigenous:{
...

	{
	  if ((uplo == blas_upper &&
		...
		if ((diag == blas_unit_diag) && (col_index == matrix_row)) {
		  {
		    rowtmp[0] = (double) vecval[0] * one;
		    rowtmp[1] = (double) vecval[1] * one;
		  }
		} else {
		  matval = tp_i[tp_index];
		  {
  ...
  case blas_prec_extra:{
...

	{
	  if ((uplo == blas_upper &&
		...
		if ((diag == blas_unit_diag) && (col_index == matrix_row)) {
		  {
		    head_rowtmp[0] = (double) vecval[0] * one;
		    tail_rowtmp[0] = 0.0;
		    head_rowtmp[1] = (double) vecval[1] * one;
		    tail_rowtmp[1] = 0.0;
		  }
		} else {
		  matval = tp_i[tp_index];

	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (incx_val = INCX_START; incx_val <= INCX_END;
			...
			FPU_FIX_STOP;
			BLAS_ssbmv(order_type,
				   uplo_type, n, k, alpha, a, lda, x, incx,
				   beta, y, incy);
			FPU_FIX_START;
		  ...
		  for (incx_val = INCX_START; incx_val <= INCX_END;
			...
			if (!(ratio <= thresh)) {
			  if (debug == 3) {
			    ...
			    printf("head_r_true\t");
			    dprint_vector(head_r_true, n * 2, 1);
			    printf("ratios :\t");
			    dprint_vector(ratios, n, 1);
			    printf("ratio = %g\n", ratio);
			  }
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_ssbmv(n, ntests, &seed, thresh, debug,
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_dsbmv(n, ntests, &seed, thresh, debug,
		  test_prob, &min_ratio, &max_ratio,
		  &num_bad_ratio, &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_csbmv(n, ntests, &seed, thresh, debug,
		  test_prob, &min_ratio, &max_ratio,
		  &num_bad_ratio, &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zsbmv(n, ntests, &seed, thresh, debug,
		  test_prob, &min_ratio, &max_ratio,
		  &num_bad_ratio, &num_tests);

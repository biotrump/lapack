	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			FPU_FIX_STOP;
			BLAS_ssymm(order_type, side_type,
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* call symm routines to be tested */
			FPU_FIX_STOP;
			BLAS_dsymm(order_type, side_type,
				   uplo_type, m, n, alpha, a, lda, b, ldb,
				   beta, c, ldc);
			FPU_FIX_START;
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* call symm routines to be tested */
			FPU_FIX_STOP;
			BLAS_csymm(order_type, side_type,
				   uplo_type, m, n, alpha, a, lda, b, ldb,
				   beta, c, ldc);
			FPU_FIX_START;
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* call symm routines to be tested */
			FPU_FIX_STOP;
			BLAS_zsymm(order_type, side_type,
				   uplo_type, m, n, alpha, a, lda, b, ldb,
				   beta, c, ldc);
			FPU_FIX_START;
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_ssymm(m, n, ntests, &seed, thresh, debug,
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_dsymm(m, n, ntests, &seed, thresh, debug,
		  test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		  &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_csymm(m, n, ntests, &seed, thresh, debug,
		  test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		  &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zsymm(m, n, ntests, &seed, thresh, debug,
		  test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		  &num_tests);

...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_chpmv(n, ntests, &seed, thresh, debug,
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhpmv(n, ntests, &seed, thresh, debug,
		  test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		  &num_tests);

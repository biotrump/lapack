	  ...
	  for (uplo_val = 0; uplo_val < 2; uplo_val++) {
		...
		for (i = 0; i < ntests; i++) {
		  ...
		  for (incx_val = -2; incx_val <= 2; incx_val++) {
		    ...
		    /* call BLAS_stpmv */
		    FPU_FIX_STOP;
		    BLAS_stpmv(order_type, uplo_type, trans_type, diag_type,
	  ...
	  for (uplo_val = 0; uplo_val < 2; uplo_val++) {
		...
		for (i = 0; i < ntests; i++) {
		  ...
		  for (incx_val = -2; incx_val <= 2; incx_val++) {
		    ...
		    /* call BLAS_dtpmv */
		    FPU_FIX_STOP;
		    BLAS_dtpmv(order_type, uplo_type, trans_type, diag_type,
			       n, alpha, tp, x, incx_val);
		    FPU_FIX_START;
			...
			for (j = 0; j < n; j++) {
			  ...
			  printf
			    ("head_r_true[%d]=(%.16e, %.16e),\n      tail_r_true[%d]=(%.16e, %.16e)",
			     j * inc_index, head_r_true[j * inc_index],
			     head_r_true[j * inc_index + 1], j * inc_index,
	  ...
	  for (uplo_val = 0; uplo_val < 2; uplo_val++) {
		...
		for (i = 0; i < ntests; i++) {
		  ...
		  for (incx_val = -2; incx_val <= 2; incx_val++) {
		    ...
		    /* call BLAS_ctpmv */
		    FPU_FIX_STOP;
		    BLAS_ctpmv(order_type, uplo_type, trans_type, diag_type,
			       n, alpha, tp, x, incx_val);
		    FPU_FIX_START;
	  ...
	  for (uplo_val = 0; uplo_val < 2; uplo_val++) {
		...
		for (i = 0; i < ntests; i++) {
		  ...
		  for (incx_val = -2; incx_val <= 2; incx_val++) {
		    ...
		    /* call BLAS_ztpmv */
		    FPU_FIX_STOP;
		    BLAS_ztpmv(order_type, uplo_type, trans_type, diag_type,
			       n, alpha, tp, x, incx_val);
		    FPU_FIX_START;
		    ...
		    for (j = 0; j < n; j++) {
		      /* copy row j of tp to temp */
		      ztpmv_copy_row(order_type, uplo_type, trans_type, n, tp,
				     temp, j);
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    n = sizes[i];
    total_max_ratio = do_test_stpmv(n, ntests, &seed, thresh, debug,
				    test_prob,
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    n = sizes[i];
    total_max_ratio = do_test_dtpmv(n, ntests, &seed, thresh, debug,
				    test_prob,
				    &total_min_ratio, &num_bad_ratio,
				    &num_tests);
    if (total_max_ratio > max_ratio)
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    n = sizes[i];
    total_max_ratio = do_test_ctpmv(n, ntests, &seed, thresh, debug,
				    test_prob,
				    &total_min_ratio, &num_bad_ratio,
				    &num_tests);
    if (total_max_ratio > max_ratio)
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    n = sizes[i];
    total_max_ratio = do_test_ztpmv(n, ntests, &seed, thresh, debug,
				    test_prob,
				    &total_min_ratio, &num_bad_ratio,
				    &num_tests);
    if (total_max_ratio > max_ratio)

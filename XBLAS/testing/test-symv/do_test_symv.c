		  ...
		  for (incy_val = INCY_START; incy_val <= INCY_END;
			...
			break;
		      ...
		      BLAS_ssymv_testgen(norm, order_type,
					 ...
					 alpha_flag, &beta, beta_flag, a, lda,
					 x, incx, y, incy, seed, head_r_true,
					 tail_r_true);
		      test_count++;
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_ssymv(n, ntests, &seed, thresh, debug,
		  test_prob,
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_dsymv(n, ntests, &seed, thresh, debug,
		  test_prob,
		  &min_ratio, &max_ratio, &num_bad_ratio, &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_csymv(n, ntests, &seed, thresh, debug,
		  test_prob,
		  &min_ratio, &max_ratio, &num_bad_ratio, &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zsymv(n, ntests, &seed, thresh, debug,
		  test_prob,
		  &min_ratio, &max_ratio, &num_bad_ratio, &num_tests);

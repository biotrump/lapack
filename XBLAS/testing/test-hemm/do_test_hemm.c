...
void do_test_chemm(int m, int n,
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			BLAS_chemm_testgen(norm, order_type,
					   uplo_type, side_type, m, n,
					   randomize_val, &alpha, alpha_flag,
					   &beta, beta_flag, a, lda, b, ldb,
					   c, ldc, seed, head_r_true,
					   tail_r_true);
			test_count++;
			...
			FPU_FIX_STOP;
			BLAS_chemm(order_type, side_type,
			...
			for (i = 0, ci = 0, ri = 0;
			  ...
			  chemm_copy_row(order_type, uplo_type, side_type,
					 m_i, a, lda, a_vec, i);
			...
			if (!(ratio <= thresh)) {
			  ...
			  if (debug == 3) {
			    ...
			    printf("head_r_true\n");
			    zprint_matrix(head_r_true, m, n, ldc, order_type);
...
void do_test_zhemm(int m, int n,
		   int ntests, int *seed, double thresh, int debug,
		   float test_prob, double *min_ratio, double *max_ratio,
		   int *num_bad_ratio, int *num_tests)
{
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* finally we are here to generate the test case */
			BLAS_zhemm_testgen(norm, order_type,
					   uplo_type, side_type, m, n,
					   randomize_val, &alpha, alpha_flag,
					   &beta, beta_flag, a, lda, b, ldb,
					   c, ldc, seed, head_r_true,
			...
			/* call hemm routines to be tested */
			FPU_FIX_STOP;
			BLAS_zhemm(order_type, side_type,
				   uplo_type, m, n, alpha, a, lda, b, ldb,
				   beta, c, ldc);
			FPU_FIX_START;
			...
			for (i = 0, ci = 0, ri = 0;
			     i < m_i; i++, ci += incci, ri += incri) {
			  zhemm_copy_row(order_type, uplo_type, side_type,
					 m_i, a, lda, a_vec, i);
			  for (j = 0, cij = ci, rij = ri;
			       j < n_i; j++, cij += inccij, rij += incrij) {
			    /* copy i-th row of A and j-th col of B */
...
void do_test_zhemm_z_c(int m, int n,
		       int ntests, int *seed, double thresh, int debug,
		       float test_prob, double *min_ratio, double *max_ratio,
		       int *num_bad_ratio, int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemm_z_c";
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* finally we are here to generate the test case */
			BLAS_zhemm_z_c_testgen(norm, order_type,
					       uplo_type, side_type, m, n,
					       randomize_val, &alpha,
...
void do_test_zhemm_c_z(int m, int n,
		       int ntests, int *seed, double thresh, int debug,
		       float test_prob, double *min_ratio, double *max_ratio,
		       int *num_bad_ratio, int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemm_c_z";
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* finally we are here to generate the test case */
			BLAS_zhemm_c_z_testgen(norm, order_type,
					       uplo_type, side_type, m, n,
					       randomize_val, &alpha,
					       alpha_flag, &beta, beta_flag,
					       a, lda, b, ldb, c, ldc, seed,
...
void do_test_zhemm_c_c(int m, int n,
		       int ntests, int *seed, double thresh, int debug,
		       float test_prob, double *min_ratio, double *max_ratio,
		       int *num_bad_ratio, int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemm_c_c";
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* finally we are here to generate the test case */
			BLAS_zhemm_c_c_testgen(norm, order_type,
					       uplo_type, side_type, m, n,
					       randomize_val, &alpha,
					       alpha_flag, &beta, beta_flag,
					       a, lda, b, ldb, c, ldc, seed,
...
void do_test_chemm_c_s(int m, int n,
		       int ntests, int *seed, double thresh, int debug,
		       float test_prob, double *min_ratio, double *max_ratio,
		       int *num_bad_ratio, int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_chemm_c_s";
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* finally we are here to generate the test case */
			BLAS_chemm_c_s_testgen(norm, order_type,
					       uplo_type, side_type, m, n,
					       randomize_val, &alpha,
					       alpha_flag, &beta, beta_flag,
					       a, lda, b, ldb, c, ldc, seed,
...
void do_test_zhemm_z_d(int m, int n,
		       int ntests, int *seed, double thresh, int debug,
		       float test_prob, double *min_ratio, double *max_ratio,
		       int *num_bad_ratio, int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemm_z_d";
	  ...
	  for (order_val = ORDER_START; order_val <= ORDER_END; order_val++) {
		...
		for (lda_val = LDA_START; lda_val <= LDA_END; lda_val++) {
		  ...
		  for (ldb_val = LDB_START; ldb_val <= LDB_END; ldb_val++) {
			...
			/* finally we are here to generate the test case */
			BLAS_zhemm_z_d_testgen(norm, order_type,
					       uplo_type, side_type, m, n,
					       randomize_val, &alpha,
					       alpha_flag, &beta, beta_flag,
					       a, lda, b, ldb, c, ldc, seed,
...
void do_test_chemm_x(int m, int n,
		     int ntests, int *seed, double thresh, int debug,
		     float test_prob, double *min_ratio, double *max_ratio,
		     int *num_bad_ratio, int *num_tests)
{
			...
			for (randomize_val = RANDOMIZE_START;
			  ...
			  if (!(ratio <= thresh)) {
				...
				printf("Not Randomized\n");
			      ...
			      printf("head_r_true\n");
			      zprint_matrix(head_r_true, m, n, ldc,
					    order_type);
...
void do_test_zhemm_x(int m, int n,
		     int ntests, int *seed, double thresh, int debug,
		     float test_prob, double *min_ratio, double *max_ratio,
		     int *num_bad_ratio, int *num_tests)
{
			...
			for (randomize_val = RANDOMIZE_START;
			  ...
			  if (!(ratio <= thresh)) {
				...
				printf("Not Randomized\n");
			      ...
			      /* print out info */
			      printf("alpha[0]=%.24e, alpha[1]=%.24e",
				     alpha[0], alpha[1]);;
			      printf("   ");
			      printf("beta[0]=%.24e, beta[1]=%.24e", beta[0],
				     beta[1]);;
...
void do_test_zhemm_z_c_x(int m, int n,
			 int ntests, int *seed, double thresh, int debug,
			 ...
			 int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemm_z_c_x";
...
void do_test_zhemm_c_z_x(int m, int n,
			 int ntests, int *seed, double thresh, int debug,
			 float test_prob, double *min_ratio,
			 double *max_ratio, int *num_bad_ratio,
			 int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemm_c_z_x";
...
void do_test_zhemm_c_c_x(int m, int n,
			 int ntests, int *seed, double thresh, int debug,
			 float test_prob, double *min_ratio,
			 double *max_ratio, int *num_bad_ratio,
			 int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemm_c_c_x";
...
void do_test_chemm_c_s_x(int m, int n,
			 int ntests, int *seed, double thresh, int debug,
			 float test_prob, double *min_ratio,
			 double *max_ratio, int *num_bad_ratio,
			 int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_chemm_c_s_x";
...
void do_test_zhemm_z_d_x(int m, int n,
			 int ntests, int *seed, double thresh, int debug,
			 float test_prob, double *min_ratio,
			 double *max_ratio, int *num_bad_ratio,
			 int *num_tests)
{
  ...
  /* Function name */
  const char fname[] = "BLAS_zhemm_z_d_x";
...
int main(int argc, char **argv)
{
  ...
  if (argc != 6) {
    printf("Usage:\n");
    printf("do_test_hemm <nsizes> <ntests> <thresh> <debug> <test_prob>\n");
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_chemm(m, n, ntests, &seed, thresh, debug,
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm(m, n, ntests, &seed, thresh, debug,
		  test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		  &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_z_c(m, n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_c_z(m, n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_c_c(m, n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_z_d(m, n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_chemm_c_s(m, n, ntests, &seed, thresh, debug,
		      test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		      &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_chemm_x(m, n, ntests, &seed, thresh, debug,
		    test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		    &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_x(m, n, ntests, &seed, thresh, debug,
		    test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		    &num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_z_c_x(m, n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_c_z_x(m, n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_c_c_x(m, n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_chemm_c_s_x(m, n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);
  ...
  for (i = 0; i < nsizes; i++) {
    ...
    do_test_zhemm_z_d_x(m, n, ntests, &seed, thresh, debug,
			test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
			&num_tests);

...
void do_test_ssum(int n, int ntests, int *seed, double thresh,
 ...
 */
{
  ...
  for (norm = NORM_START; norm <= NORM_END; norm++) {
	...
	for (j = 0, x_gen_i = 0; j < n; j++, x_gen_i += incx_gen, xi += incx) {
	  x_elem = x_gen[x_gen_i];
	  x[xi] = x_elem;
	}

	FPU_FIX_STOP;
	BLAS_ssum(n, x, incx_val, &sum);
	FPU_FIX_START;
...
void do_test_dsum(int n, int ntests, int *seed, double thresh,
		  int debug, float test_prob, double *min_ratio,
		  double *max_ratio, int *num_bad_ratio, int *num_tests)
...
void do_test_csum(int n, int ntests, int *seed, double thresh,
		  int debug, float test_prob, double *min_ratio,
		  double *max_ratio, int *num_bad_ratio, int *num_tests)
 ...
 */
{
  ...
  for (norm = NORM_START; norm <= NORM_END; norm++) {
	...
	BLAS_csum_testgen(n, norm, x_gen, seed, head_sum_true, tail_sum_true);
...
void do_test_zsum(int n, int ntests, int *seed, double thresh,
		  int debug, float test_prob, double *min_ratio,
		  double *max_ratio, int *num_bad_ratio, int *num_tests)
 ...
 */
{
  ...
  for (norm = NORM_START; norm <= NORM_END; norm++) {
	...
	BLAS_zsum_testgen(n, norm, x_gen, seed, head_sum_true, tail_sum_true);
...
int main(int argc, char **argv)
{
  ...
  for (n = 0; n <= nsizes; n++) {
    do_test_ssum(n, ntests, &seed, thresh, debug,
  ...
  for (n = 0; n <= nsizes; n++) {
    do_test_dsum(n, ntests, &seed, thresh, debug,
		 test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		 &num_tests);
    if (debug == 2 || (debug == 1 && num_bad_ratio > 0)) {
      printf("  n = %d:  ", n);
  ...
  for (n = 0; n <= nsizes; n++) {
    do_test_csum(n, ntests, &seed, thresh, debug,
		 test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		 &num_tests);
    if (debug == 2 || (debug == 1 && num_bad_ratio > 0)) {
      printf("  n = %d:  ", n);
  ...
  for (n = 0; n <= nsizes; n++) {
    do_test_zsum(n, ntests, &seed, thresh, debug,
		 test_prob, &min_ratio, &max_ratio, &num_bad_ratio,
		 &num_tests);
    if (debug == 2 || (debug == 1 && num_bad_ratio > 0)) {
      printf("  n = %d:  ", n);
